...
module.exports = class KeyCloakService {
  ...
  async updateUser(id, email, roles, status) {
    try {
      ...
      if (!user) {
        throw new APIerror(
          404,
          "Keycloak updateUser : user not found",
          "User not found"
        );
      }
      ...
      if (!newGroup) {
        throw new APIerror(
          400,
          "Keycloak updateUser : group requested not available",
  ...
  async selfResetPassword(userId) {
    ...
    try {
      ...
      throw new APIerror(
        400,
        "Keycloak Admin API: executeActionsEmail failed",
  ...
  async loginUser(email, password) {
    try {
      ...
      const response = await axios.post(
        `${this.kcBase}/realms/${this.kcRealm}/protocol/openid-connect/token`,
        {
  ...
  async selfUpdateUser(userId, email, accessToken) {
    try {
      ...
      const response = await axios.post(
        `${this.kcBase}/realms/${this.kcRealm}/account/`,
        {
          email,
        },
        {
      ...
      if (response.status !== 204) {
        throw new APIerror(
          400,
          "keycloak update email: response is not 204",
          "Failed to update user"
        );
      }
