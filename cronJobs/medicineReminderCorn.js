...
const DataBase = require("../cronJobs/presDatabaseCorn");
...
cron.schedule("* * * * *", async () => {
  try {
    ...
    const sendReminders = async () => {
      ...
      const getUserPres = await DataBase.findAllUserPrescriptions(
        todayDate,
        currentTime
      );
      const getDependPres = await DataBase.findAllDependPrescriptions(
        todayDate,
        currentTime
      );
      ...
      for (let w = 0; w < getUserPres.length; w++) {
        let getUserPresNames = await DataBase.findUserPrescriptionFNames(
          getUserPres[w].userID
        );
      ...
      for (let w = 0; w < getDependPres.length; w++) {
        let getDependPresNames = await DataBase.findDependPrescriptionFNames(
          getDependPres[w].dID
        );
      ...
      if (getUserPres.length > 0) {
        for (let i = 0; i < getUserPres.length; i++) {
          let getUserTokens = await DataBase.findUserActiveFcmTokens(
            getUserPres[i].userID
          );
      ...
      if (getDependPres.length > 0) {
        for (let i = 0; i < getDependPres.length; i++) {
          let getUserTokens = await DataBase.findUserActiveFcmTokens(
            getDependPres[i].userID
          );
          if (getUserTokens.length == 1) {
            for (let j = 0; j < getUserTokens[0].TokenStores.length; j++) {
              TokenStorageDepend.push(
                getUserTokens[0].TokenStores[j].fcm_token
              );
          ...
          for (let j = 0; j < userIDStorageUser.length; j++) {
            if (userIDStorageUser[j] == getId) {
              for (let z = 0; z < getUserPres[i].presMedicine.length; z++) {
                ...
                finalPatientNameList.push(userPreNameStorage[i]);
                finalMedMealTimeList.push(
                  getUserPres[i].presMedicine[z].mealTiming
                );
              }
            }
          }
          ...
          for (let j = 0; j < userIDStorageDepend.length; j++) {
            if (userIDStorageDepend[j] == getId) {
              for (let z = 0; z < getDependPres[i].presMedicine.length; z++) {
                ...
                finalTokenList.push(TokenStorageDepend[j]);
                ...
                finalMedMealTimeList.push(
                  getDependPres[i].presMedicine[z].mealTiming
                );
                finalPatientNameList.push(dependPreNameStorage[i]);
              }
            }
          }
        }
      ...
      if (finalTokenList.length > 0) {
        ...
        for (let x = 0; x < finalTokenList.length; x++) {
          const dataObject = {
            ...
            quantity: finalMedQtyList[x].toString(),
